...
$query=mysqli_query($con,"insert into tblcomplaints(userId,category,subcategory,complaintType,state,noc,complaintDetails,complaintFile,userEmail) values('$uid','$category','$subcat','$complaintype','$state','$noc','$complaintdetials','$compfile','$userEmail')");
...
<div class="col-sm-4">
<?php $sql=mysqli_query($con,"select * from users where userEmail='".$_SESSION['login']."'");
while ($rw=mysqli_fetch_array($sql)) {
  ?>
<input type="text" name="subcategory" required="required" value="<?php echo htmlentities($rw['department']);?>" readonly="" class="form-control">
...
<div class="form-group">
<label class="col-sm-2 col-sm-2 control-label">School Code</label>
<div class="col-sm-4">
<?php $sql=mysqli_query($con,"select * from users where userEmail='".$_SESSION['login']."'");
while ($rw=mysqli_fetch_array($sql)) {
  ?>
<input type="text" name="category" required="required" value="<?php echo htmlentities($rw['school']);?>" readonly="" class="form-control">
...
<div class="col-sm-4">
<?php $sql=mysqli_query($con,"select userEmail from users where userEmail='".$_SESSION['login']."'");
while ($rw=mysqli_fetch_array($sql)) {
  ?>
<input type="text" name="userEmail" required="required" value="<?php echo htmlentities($rw['userEmail']);?>" readonly="" class="form-control">
